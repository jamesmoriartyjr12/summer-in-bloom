...
export function PortfolioCompanyRow({
  ...
  const shouldReveal = reduceMotion || inView || isOverlay;
  ...
  const contentBlock = (
    <>
      <motion.div
        className={isOverlay ? "pt-0" : "desktop:pt-[32px] pt-0"}
        variants={revealVariants}
        initial={reduceMotion || isOverlay ? false : "hidden"}
        animate={shouldReveal ? "visible" : "hidden"}
        transition={{
          duration: reduceMotion ? 0 : 0.5,
          delay: reduceMotion || isOverlay ? 0 : staggerBase,
      ...
      <motion.div
        ...
        transition={{
          duration: reduceMotion ? 0 : 0.5,
          delay: reduceMotion || isOverlay ? 0 : staggerBase + 0.06,
          ease: PORTFOLIO_IMAGE_EASE,
        }}
      >
        <div
      ...
      {!article ? (
        <motion.div
          className={isOverlay ? "pb-0" : "pb-[28px] border-b border-beige"}
          variants={revealVariants}
          initial={reduceMotion || isOverlay ? false : "hidden"}
          animate={shouldReveal ? "visible" : "hidden"}
          transition={{
            duration: reduceMotion ? 0 : 0.5,
            delay: reduceMotion || isOverlay ? 0 : staggerBase + 0.12,
            ease: PORTFOLIO_IMAGE_EASE,
          }}
        >
        ...
        <motion.div
          className={`flex flex-col desktop:flex-row desktop:items-end gap-[16px] desktop:gap-[24px] ${isOverlay ? "pb-0" : "pb-[28px] border-b border-beige"}`}
          variants={revealVariants}
          initial={reduceMotion || isOverlay ? false : "hidden"}
          animate={shouldReveal ? "visible" : "hidden"}
          transition={{
  ...
  if (isOverlay) {
    return (
      <motion.div
        ...
        animate={{
          opacity: reduceMotion ? 1 : isActive ? 1 : 0.25,
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={
          shouldReveal ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 1.03 }
        }
        transition={{
          duration: reduceMotion ? 0 : 0.5,
          ease: PORTFOLIO_IMAGE_EASE,
